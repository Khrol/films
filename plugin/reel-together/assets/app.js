  ...
  function openEntry(entry = null, editList = false, copy = false) {
    ...
    const renderSharing = () => {
      ...
      target.querySelectorAll('input').forEach(input => input.addEventListener('change', () => {
        if (input.checked) sharedCompanions.add(Number(input.value)); else sharedCompanions.delete(Number(input.value));
      }));
    };
    const renderChoices = () => {
      const companions = state.data.companions.filter(c => c.can_edit);
  ...
  root.addEventListener('submit', async event => {
    if (event.target.id === 'filter-form') {
      ...
      try {
        ...
        if (id) {
          body.linked_user_id = Number(form.elements.namedItem('linked_user_id').value);
