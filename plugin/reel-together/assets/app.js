(() => {
  ...
  const state = { view: 'watched', scope: 'all', companion: 'all', query: '', page: 1, data: null, items: [], request: 0 };
  ...
  async function api(path, method = 'GET', body) {
    // Preserve ?rest_route= on sites without pretty permalinks.
    const base = new URL(window.ReelTogether.api);
    const [route, query] = path.split('?');
    if (base.searchParams.has('rest_route')) {
      base.searchParams.set('rest_route', base.searchParams.get('rest_route') + route);
    } else base.pathname += route;
    new URLSearchParams(query || '').forEach((v, k) => base.searchParams.set(k, v));
    const response = await fetch(base, {
      method, credentials: 'same-origin', cache: 'no-store',
      headers: { 'X-WP-Nonce': window.ReelTogether.nonce, 'Content-Type': 'application/json' },
      body: body === undefined ? undefined : JSON.stringify(body),
    });
    let data;
    try { data = await response.json(); } catch { throw new Error('The server did not respond as expected. Please try again.'); }
  ...
  function syncAccount() {
    ...
    document.getElementById('count-watched').textContent = data.counts.watched;
  ...
  function openEntry(entry = null, editList = false, copy = false) {
    ...
    dialog.innerHTML = `<form id="entry-form"><header class="dialog-heading"><div><p class="eyebrow">ADD TO YOUR STORY</p><h2 id="dialog-title">${editing && entry.status === 'watched' ? 'A night to remember.' : status === 'watched' ? 'What did you watch?' : 'Save it for movie night.'}</h2></div><button class="close-button" type="button" aria-label="Close dialog">×</button></header>
      ...
      <fieldset class="movie-links"><legend>Movie links <span>(optional)</span></legend>
        ...
        <p class="helper">${state.data.catalog_enabled ? 'Load details to fill the title, ratings, and matching IDs. You can also save links with a title you enter below.' : 'Links work without a catalog connection. Enter the film title below to save. To load details and ratings, the site owner can connect a catalog in Settings → Reel Together.'}</p><div id="lookup-result" aria-live="polite"></div>
      </fieldset>
    ...
    const renderChoices = () => {
      ...
      choices.querySelectorAll('input').forEach(input => input.addEventListener('change', () => {
        if (input.checked) selectedCompanions.add(Number(input.value)); else selectedCompanions.delete(Number(input.value));
      }));
      const suggestions = ['My wife', 'My elder son'].filter(name => !companions.some(c => c.name.toLowerCase() === name.toLowerCase()));
    ...
    const updateFields = () => {
      ...
      document.getElementById('privacy-note').textContent = field('scope').value === 'personal' ? 'Only you can see this entry, including its notes and rating.' : 'Everyone in your household can see this entry, including its notes and rating.';
    };
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        dialog.close(); state.view = body.status; state.page = 1; state.companion = 'all';
  ...
  root.addEventListener('click', async event => {
    ...
    try {
      if (button.dataset.view) {
        state.view = button.dataset.view; state.page = 1; state.query = ''; state.scope = 'all'; state.companion = 'all';
        await refresh();
        ...
        const entry = state.items.find(item => item.id === Number(button.dataset.edit || button.dataset.editList || button.dataset.copy));
  ...
  root.addEventListener('submit', async event => {
    if (event.target.id === 'filter-form') {
      ...
      try {
        ...
        await api(id ? `companions/${id}` : 'companions', id ? 'PUT' : 'POST', { name: form.elements.namedItem('name').value });
        toast(id ? 'Companion updated on your viewings.' : 'Companion added.');
        await refresh();
